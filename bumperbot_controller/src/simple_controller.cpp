    ...
    
{
    ...
    declare_parameter("wheel_seperation", 0.017);

    wheel_radius = get_parameter("wheel_radius").as_double();
    wheel_seperation = get_parameter("wheel_seperation").as_double();

    RCLCPP_INFO_STREAM(get_logger(), "using wheel radius ");
    RCLCPP_INFO_STREAM(get_logger(), "using wheel seperation " << wheel_seperation);
